...
public class PriceReader extends Application implements Runnable {
    ...
    public void readSendPrice(List<String> marketIds, String pSessionToken) throws APINGException, DBConnectionException, BetOfferException {
        ...
        if (!marketIdsNew.isEmpty()) {
             ...
             * each market a readMatch call will be made to Betprover. After the
             * first, insert case, everything will be cached so the overhead
             * cost of doing this is only for the first call in order to keep
             * the design exactly the same for Pricereader and readPrice.
             */
